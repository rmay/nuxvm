...
// -----------------------------------------------------------------------------
// Go Reference Compiler Cross-Validation
// -----------------------------------------------------------------------------
// These tests compile the same Lux source with both the C compiler and the
// official Go reference compiler (via `go run ./cmd/luxc`), then execute
// BOTH resulting programs on the C VM and compare observable behavior:
//   - Captured output from "." and EMIT
//   - Final data stack contents
//   - Clean halt
//
// This is the strongest practical way to ensure the C port is semantically
...
static void snapshot_vm(VM* vm, VMState* out) {
    memset(out, 0, sizeof(*out));
    out->halted = vm->halted;
    out->stack_len = vm->stack_ptr;
    if (out->stack_len > 64) out->stack_len = 64;
    for (int i = 0; i < out->stack_len; i++) {
        out->stack[i] = vm->stack[i];
    }
    strncpy(out->output, output_buffer, sizeof(out->output)-1);
}

static bool states_match(const VMState* a, const VMState* b, const char* test_name) {
    bool ok = true;
    if (a->halted != b->halted) {
        printf("  [%s] HALT mismatch: C=%d Go=%d\n", test_name, a->halted, b->halted);
        ok = false;
    }
    if (a->stack_len != b->stack_len) {
        printf("  [%s] Stack depth mismatch: C=%d Go=%d\n", test_name, a->stack_len, b->stack_len);
        ok = false;
    } else {
        for (int i = 0; i < a->stack_len; i++) {
            if (a->stack[i] != b->stack[i]) {
                printf("  [%s] Stack[%d] mismatch: C=%d Go=%d\n", test_name, i, a->stack[i], b->stack[i]);
                ok = false;
            }
        }
    }
    if (strcmp(a->output, b->output) != 0) {
        printf("  [%s] Output mismatch:\n    C:  \"%s\"\n    Go: \"%s\"\n", test_name, a->output, b->output);
        ok = false;
    }
    return ok;
}

static bool go_available(void) {
    // Quick check: can we invoke go and does the module look buildable?
    int rc = system("go version >/dev/null 2>&1");
    if (rc != 0) return false;
    // Also verify the cmd/luxc exists in the expected location relative to cwd
    struct stat st;
    if (stat("cmd/luxc/main.go", &st) != 0) return false;
    return true;
}

// Compile source using the Go reference compiler (headless base).
// Returns malloc'd path to the .bin on success, or NULL on failure.
// Caller must free the returned string.
static char* compile_with_go_reference(const char* source) {
    static int counter = 0;
    char lux_path[256];
    char bin_path[256];

    snprintf(lux_path, sizeof(lux_path), "/tmp/lux_go_ref_%d_%d.lux", (int)getpid(), counter++);
    snprintf(bin_path, sizeof(bin_path), "/tmp/lux_go_ref_%d_%d.bin", (int)getpid(), counter-1);

    FILE* f = fopen(lux_path, "wb");
    if (!f) return NULL;
    ...
    snprintf(cmd, sizeof(cmd),
             "go run ./cmd/luxc -target headless -o '%s' '%s' >/dev/null 2>&1",
             bin_path, lux_path);

    int rc = system(cmd);

    unlink(lux_path); // clean up source
    ...
    struct stat st;
    if (stat(bin_path, &st) != 0 || st.st_size < 6) {
        unlink(bin_path);
        return NULL;
    }
...
static void run_go_cross_case(const char* name, const char* source) {
    printf("  Cross: %s ... ", name);
    fflush(stdout);
    ...
    reset_output();
    size_t c_len = 0;
    uint8_t* c_bc = compile_source(source, HEADLESS_BASE_ADDRESS, &c_len, false);
    if (!c_bc) {
        printf("C compile FAILED\n");
        return;
    }
    VM* c_vm = run_and_capture(c_bc, c_len, true);
    VMState c_state;
    snapshot_vm(c_vm, &c_state);
    vm_free(c_vm);
    free(c_bc);

    // Go reference side
    char* go_bin_path = compile_with_go_reference(source);
    if (!go_bin_path) {
        printf("Go reference compile FAILED (skipped)\n");
        return;
    }

    // Load the Go-produced bytecode and run it on our C VM
    FILE* bf = fopen(go_bin_path, "rb");
    if (!bf) {
        printf("Failed to open Go .bin\n");
        unlink(go_bin_path);
        free(go_bin_path);
        return;
    }
    fseek(bf, 0, SEEK_END);
    long go_len = ftell(bf);
    fseek(bf, 0, SEEK_SET);
    uint8_t* go_bc = malloc(go_len);
    fread(go_bc, 1, go_len, bf);
    fclose(bf);

    reset_output();
    VM* go_vm = run_and_capture(go_bc, go_len, true);
    VMState go_state;
    snapshot_vm(go_vm, &go_state);
    vm_free(go_vm);
    free(go_bc);
    unlink(go_bin_path);
    free(go_bin_path);

    if (states_match(&c_state, &go_state, name)) {
        printf("MATCH\n");
    } else {
        printf("MISMATCH (see details above)\n");
        // Do not hard-fail the whole suite — this is diagnostic
    }
}

static void test_go_cross_validation(void) {
    printf("\n--- Go Reference Compiler Cross-Validation ---\n");

    if (!go_available()) {
        printf("Go toolchain not available or cmd/luxc missing — skipping cross-validation.\n");
        return;
    }

    // Curated set of programs that exercise the features we ported.
    // All use HEADLESS semantics.

    run_go_cross_case("basic-arithmetic", "5 7 + 3 * .");

    run_go_cross_case("word-def", "@sq dup * ; 6 sq .");

    run_go_cross_case("quotation-call", "[ 42 ] CALL");

    run_go_cross_case("if-else", "1 [ 100 . ] [ 200 . ] ?: 0 [ 100 . ] [ 200 . ] ?:");

    run_go_cross_case("while", "4 [ dup 0 > ] [ dup . 1 - ] |:");

    run_go_cross_case("times", "[ 9 . ] 3 #:");

    run_go_cross_case("module-basic",
        "MODULE MATH\n"
        "@cube dup dup * * ;\n"
        "MODULE MAIN\n"
        "2 MATH::cube .");

    run_go_cross_case("import-alias",
        "MODULE LIB\n"
        "@inc 1 + ;\n"
        "MODULE APP\n"
        "IMPORT LIB AS L\n"
    ...
    run_go_cross_case("string-basic", "T\"hi\" drop 99 .");  // just test it doesn't explode + produces output

    run_go_cross_case("recursion-tro",
        "@fact\n"
        "  dup 1 > [ dup 1 - fact * ] [ drop 1 ] ?:\n"
        ";\n"
        "5 fact .");
